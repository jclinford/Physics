...
{
	private int maxIterations;		// Max number of interations allowed when resolving
...
//		System.out.println("Num contacts: " + contacts.size());
		...
		/* Resolve contacts on priority of largest closing velocity to smallest closing velocity */
		while (curIteration < maxIterations)
		{
			...
			for (int i = 0; i < contacts.size(); i++)
			{
				...
				if (sepVelocity < maxV && sepVelocity < 0 && contacts.get(i).penetration() > 0)
				{
					maxV = sepVelocity;
					maxIndex = i;
				}
			...
			Vector2 moveA = contacts.get(maxIndex).particleMovement(contacts.get(maxIndex).a());
			Vector2 moveB = contacts.get(maxIndex).particleMovement(contacts.get(maxIndex).b());

			for (int i = 0; i < contacts.size(); i++)
			{
				if (contacts.get(i).a() == contacts.get(maxIndex).a())
				{
					float newPen = contacts.get(i).penetration() - moveA.dot(contacts.get(i).normal());
					contacts.get(i).setPenetration(newPen);
				}
				else if (contacts.get(i).a() == contacts.get(maxIndex).b())
				{
					float newPen = contacts.get(i).penetration() - moveB.dot(contacts.get(i).normal());
					contacts.get(i).setPenetration(newPen);
				}
