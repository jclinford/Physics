...
public class Contact
{
	...
	private float restitution; // Coefficient of Restitution (1 = perfect
								// elastic, 0 = inelastic)
	private float penetration; // How much the two objects are penetrating one
								// another
	...
	{
		/*
		 * if the objects are already moving apart (Vs > 0), we do not need to
		 * resolve
		 */
		float velAlongNorm = velocityAlongNormal();
		if (velAlongNorm > 0)
		{
			// System.out.println("Objects moving away, not resolving velocity. Separating v: "
			// + velAlongNorm + "   v1:" + a.velocity() + "    v2:" +
			// b.velocity());
			return;
		}

		/*
		 * If we have an infinite mass system, impulses have no affect, no need
		 * to resolve
		 */
		...
		// initial momentum
		// float pi = (a.mass() * a.velocity().magnitude()) + (b.mass() *
		...
		// final momentum
		// float pf = (a.mass() * a.velocity().magnitude()) + (b.mass() *
		// b.velocity().magnitude());
		// System.out.println("Vaf: " + Va + "   Vbf: " + Vb +
...
// a.addAngularVelocity(contactNormal.dot(a.inverseMoment()) *
// impulsiveTorqueA);
//
// b.addVelocity(impulseWorld.multiplyBy(b.inverseMass()));
// b.addAngularVelocity(contactNormal.dot(b.inverseMoment()) *
// impulsiveTorqueB);
// }
//
// /** Resolve the angular component of the impulse calculated in
// updateInternals */
...
//
// if (impulsiveTorqueA < Globals.EPSILON && impulsiveTorqueA >
// -Globals.EPSILON)
// impulsiveTorqueA = 0;
// if (impulsiveTorqueB < Globals.EPSILON && impulsiveTorqueB >
// -Globals.EPSILON)
// impulsiveTorqueB = 0;
//
// a.addAngularVelocity(contactNormal.dot(a.inverseMoment()) *
...
// Vector2 velocity =
// relPos.multiplyBy(body.angularVelocity()).addTo(body.velocity());
