...
{
	/*
	 * Check for a collision. If one is found we return a Contact, otherwise we
	 * return null
	 */
	...
	private static boolean contactsContainContact(ArrayList<Contact> contacts, RigidBody a,
			RigidBody b)
	{
	...
	private static void circlePolygon(Circle circle, PolyBody polygon, ArrayList<Contact> contacts)
	{
		...
		// Now fine the closest point on the polygon's edge to the circles
		// center
	...
	/*
	 * Returns the contact point using the Minkowski Difference for polygon v
	 * polygon
	 */
	private static Contact polygonPolygon(PolyBody a, PolyBody b)
	{
		boolean contactAtA = false; // flag to denote if the contact normal is
									// in a's perspective
			...
			for (int j = 0; j < supportVertices.size(); j++)
			{
				...
				if (dist > leastPenetratingDist)
				{
					...
					if (supportVertices.size() > 1)
					{
						...
						contactPoint = b.verticesWorld().get(index0)
								.midPoint(b.verticesWorld().get(index1));
					} else
					{
		...
		for (int i = 0; i < b.numVertices(); i++)
		{
			...
			// gather b's edge
			Vector2 v0 = vertices.get(i);
			// modulu in case we go over to loop back around
			Vector2 v1 = vertices.get(((i + 1) % b.numVertices()));
			...
			for (int j = 0; j < supportVertices.size(); j++)
			{
				...
				// if we get a positive we can bail early, this means there is a
				// separating axis
				if (dist > 0)
				{
					return null;
	...
	{
		ArrayList<SupportVertex> supportVertices = new ArrayList<SupportVertex>(
				body.polygon().npoints + 1);
		...
		supportVertices.add(new SupportVertex(body.state().TransformBy(
				body.vertices().get(closestI)), closestI));
		if (secondClosestI != -1)
			supportVertices.add(new SupportVertex(body.state().TransformBy(
...
//
// StateMatrix mA = new
// StateMatrix(a.center().addTo(a.velocity().multiplyBy(t)),
// a.orientation() + (a.angularVelocity() * t));
// StateMatrix mB = new
// StateMatrix(b.center().addTo(b.velocity().multiplyBy(t)),
...
// * Does this by comparing distance of vertex's to center. The vertice that is
// closest
// * to the center should be the contact point. If two vertices are equal
// distance away then
// * we have an edge/edge collision, and the midpoint of these two vertices will
// be returned */
...
// // System.out.println("Penetration: " + dist + "   eV" + (j +
// 1)%a.numVertices() + ":" + v1 + "  eV" + j + ":" + v2 + "   v:" +
// vertices.get(i));
//
...
// }
//
// // now check for vertex's of b against a if we didn't find a negative value
// above
...
// return;
//
// Contact contact1 = new Contact(a, b, Globals.DEFAULT_RESTITUTION,
// minDistance1.magnitude());
// contact1.setContactPoint(contactPoint1);
// normal1.rotateThis90(); // this normal is just V1-V2, need to rotate and
// normalize to get actual normal
// normal1.normalize();
// contact1.setNormal(normal1);
// contacts.add(contact1);
// // System.out.println(" a: " + a + "     b: " + b);
// // System.out.println("Contact1 : " + contactPoint1 + "   pen" +
// minDistance1);
//
//
// // if we have a second contact (edge/edge collision) then we need to add it
// to the list
// if (contactPoint2 != null && normal2 != null && minDistance2 != null)
// {
// Contact contact2 = new Contact(a, b, Globals.DEFAULT_RESTITUTION,
// minDistance2.magnitude());
...
//
// // System.out.println("Contact2 : " + contactPoint2 + "   pen" +
// minDistance2);
// }
//
// return;
...
// PolyBody a = (PolyBody) aR;
// PolyBody b = (PolyBody) bR;
// ArrayList<Vector2> axes = new ArrayList<Vector2>(32); // holds all the
// normals we calculate
//
...
// private static boolean axisSeparatePolygons(Vector2 axis, PolyBody a,
// PolyBody b)
// {
